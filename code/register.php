...
try {
    ...
    // ===== provider関連 =====
    if ($is_provider) {
        // ★ フォームの min_age / max_age を取得
        $childAgeMin = isset($provider['min_age']) && $provider['min_age'] !== ''
            ? (int)$provider['min_age'] : null;
        $childAgeMax = isset($provider['max_age']) && $provider['max_age'] !== ''
            ? (int)$provider['max_age'] : null;

        // provider_profiles
        $sqlProf = '
          INSERT INTO provider_profiles
            (member_id,car_allowed,insurance_status,pets_notes,remarks,child_age_min,child_age_max)
          VALUES
            (:member_id,:car_allowed,:insurance_status,:pets_notes,:remarks,:child_age_min,:child_age_max)
        ';
        ...
        $stmtProf->bindValue(':child_age_min', $childAgeMin, PDO::PARAM_INT);
        $stmtProf->bindValue(':child_age_max', $childAgeMax, PDO::PARAM_INT);
        $stmtProf->execute();
        ...
        if (!empty($provider['activities']) && is_array($provider['activities'])) {
            ...
            $activityOther = nullif_empty($provider['activity_other_text'] ?? null);
            ...
            foreach ($provider['activities'] as $label) {
                ...
                // 「その他」がチェックされているときだけ自由記述を保存
                $stmtPA->bindValue(':other_text', ($label === 'その他') ? $activityOther : null);
                $stmtPA->execute();
            }
        }
